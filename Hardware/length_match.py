...
(C) 2021 Piotr Esden-Tempski @esden
...
def print_rgbcolor(r, g, b, s):
    print("\x1b[48;2;{};{};{}m{}\x1b[0m".format(r,g,b,s))
...
def match_netgroup(annotation: Annotation, nets: Sequence[Net]) -> bool:
    ...
    print_color(BRIGHTGREEN if meets else BRIGHTRED,
                f"{annotation.id} {'MEETS' if meets else 'FAILS'} TOL {nm_to_mm(annotation.tolerance):.3f}mm (maxΔ {nm_to_mm(delta):.3f}mm)")
    ...
    for net in nets:
        ...
        via_string = f' {net.viacount:2d}via' if net.viacount > 0 else ''
        print_rgbcolor(round(200 * (from_max / (-nm_to_mm(delta)))), 0, 0,
            f"  {net.netname:^{alignment}s} {actual_length:6.3f}mm  (Δmed {from_median: 6.3f}mm | Δmax {from_max: 6.3f}mm){via_string}")
...
def match_netgroup_reflength(annotation: Annotation, nets: Sequence[Net], ref_length: float) -> bool:
    ...
    print_color(BRIGHTGREEN if meets else BRIGHTRED,
                f"{annotation.id} {'MEETS' if meets else 'FAILS'} TOL {nm_to_mm(annotation.tolerance):.3f} mm (maxΔ {nm_to_mm(delta):.3f}mm | ref {nm_to_mm(ref_length):.3f}mm)")
    ...
    for net in nets:
        ...
        print_rgbcolor(round(200 * (from_reference / abs(nm_to_mm(delta)))), 0, 0,
...
def parse_schematic_annotation(filepath) -> Sequence[Annotation]:
    ...
    for text in text_elements:
        texts += [text.encode('utf-8').decode('unicode_escape')]
