...
void USB::InterruptHandler()					// In Drivers\STM32H7xx_HAL_Driver\Src\stm32h7xx_hal_pcd.c
{
				...
				if ((epint & USB_OTG_DIEPINT_TXFE) == USB_OTG_DIEPINT_TXFE) {			// 0x80 Transmit FIFO empty
					...
					if (epnum == 0) {
						...
						if (classByEP[epnum]->inBuffSize <= classByEP[epnum]->inBuffCount) {
							const uint32_t fifoemptymsk = (0x1UL << (epnum & EP_ADDR_MASK));
							USBx_DEVICE->DIEPEMPMSK &= ~fifoemptymsk;
						}
					}
				}
	...
	if (ReadInterrupts(USB_OTG_GINTSTS_USBRST))	{
		...
		while (count++ < usbTimeout && (USB_OTG_FS->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL) == 0);		// Wait for AHB master IDLE state.
		...
		// Clear Default Address (will be set later when address instruction received from host)
		USBx_DEVICE->DCFG &= ~USB_OTG_DCFG_DAD;

		// Set up EP0 to receive SETUP packets
...
void USB::Init(bool softReset)
{
	...
	if (!softReset) {
		...
		RCC->D2CCIP2R |= RCC_D2CCIP2R_USBSEL;			// Set the USB CLock MUX to hsi48_ck
	...
	count = 0;
//	while (count++ < 0xF000000 && (USB_OTG_FS->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL) == 0);		// Wait for AHB master IDLE state.
	USB_OTG_FS->GRSTCTL |= USB_OTG_GRSTCTL_CSRST;
	count = 0;
	while (count++ < 0xF000000 && ((USB_OTG_FS->GRSTCTL & USB_OTG_GRSTCTL_CSRST) == USB_OTG_GRSTCTL_CSRST));
	...
	count = 0;
	while (count++ < 0xF000000 && (USB_OTG_FS->GINTSTS & USB_OTG_GINTSTS_CMOD));	// CMOD = 0 is device mode
	...
	count = 0;
	while (count++ < 0xF000000 && ((USB_OTG_FS->GRSTCTL & USB_OTG_GRSTCTL_TXFFLSH) == USB_OTG_GRSTCTL_TXFFLSH));

	USB_OTG_FS->GRSTCTL = USB_OTG_GRSTCTL_RXFFLSH;		// Flush the RX buffers
	count = 0;
	while (count++ < 0xF000000 && ((USB_OTG_FS->GRSTCTL & USB_OTG_GRSTCTL_RXFFLSH) == USB_OTG_GRSTCTL_RXFFLSH));
	...
	// Endpoint 3 FIFO size/address (address is offset from EP2 address+size above)
...
void USB::ReadPacket(const uint32_t* dest, const uint16_t len, const uint32_t offset)
{
	...
	const uint32_t count32b = ((uint32_t)len + 3U) / 4U;
...
void USB::SendString(const std::string s)
{
	SendString(s.c_str());
}
