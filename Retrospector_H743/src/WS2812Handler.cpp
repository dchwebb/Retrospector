...
void WS2812Handler::LEDSet(ledName l, uint8_t b)
{
	...
	if (colours[l] != oldColours[l]) {
		...
		tmp = (tmp << 1) | 0x924924;		// insert the 1s so the pattern is 1x01x01x01x0 etc
...
void WS2812Handler::LEDColour(uint8_t leds, uint32_t rgb)
{
	 ...
	 * Technique effectively uses fact that a bit in NRZ format can be treated as 3 SPI bits: bits 'abc' becomes '1a0 1b0 1c0'
